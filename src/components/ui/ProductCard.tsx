...
import { 
  Heart, Eye, ShoppingBag, Star, TrendingUp 
} from 'lucide-react'
...
const ProductCard = memo(({ 
  ...
  return (
    <motion.div
      className={`group relative ${cardSizes[size]}`}
    ...
    >
      <div className="relative bg-white rounded-3xl overflow-hidden shadow-card hover:shadow-card-hover transition-all duration-500 border border-gray-100">
          ...
          {showQuickActions && (
            ...
            >
              <div className="absolute bottom-4 left-4 right-4 flex justify-between items-end">
                <motion.button
                  className="p-3 bg-white/90 backdrop-blur-sm rounded-2xl text-gray-700 hover:text-amber-600 transition-colors shadow-lg"
                ...
                <motion.button
                  className={`p-3 bg-white/90 backdrop-blur-sm rounded-2xl transition-all duration-300 shadow-lg ${
        ...
        <div className="p-6 space-y-4">
          ...
          {/* Add to Cart Button */}
          <motion.button
            className="w-full py-3.5 bg-amber-500 text-white rounded-2xl font-semibold hover:bg-amber-600 transition-all duration-300 flex items-center justify-center gap-2 shadow-lg hover:shadow-xl"
